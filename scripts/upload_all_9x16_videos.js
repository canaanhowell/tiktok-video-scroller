#!/usr/bin/env node
// Upload all 9x16 videos from staging input folders with proper metadata
  ...
  function searchDir(dir) {
    ...
    items.forEach(item => {
      ...
      if (stat.isDirectory() && !item.startsWith('.')) {
        ...
        // Check if it's likely 9x16 (not 16x9)
        ...
        const is9x16 = item.includes('9x16') || item.includes('9_16') || 
                      ...
                      item.includes('mobile') || item.includes('shorts');
        const is16x9 = item.includes('16x9') || item.includes('16_9') || 
                      item.includes('landscape') || item.includes('horizontal');
        
        // Include if explicitly 9x16 or not explicitly 16x9
        if (is9x16 || (!is16x9 && !item.includes('16x9'))) {
          videos.push({
            path: fullPath,
            filename: item,
...
async function uploadAll9x16Videos() {
  console.log('📱 Uploading All 9x16 Videos from Input Folders\n');
  console.log('=' .repeat(60));
  
  const videos = find9x16Videos();
  console.log(`\n📊 Found ${videos.length} potential 9x16 videos\n`);
  ...
  if (uploaded > 0) {
    ...
    console.log('\n🔍 Test mobile view at: https://media.synthetikmedia.ai');
  }
}

// Run the upload
uploadAll9x16Videos().catch(console.error);